...
public class MechanicalPowerToolAuthoritySystem extends BaseComponentSystem {
    ...
    static List<Vector3i> getPotentialBlockPlacementPositions(ToolBlockPlacementComponent blockPlacementComponent,
                                                              EntityRef characterEntity, WorldProvider worldProvider,
                                                              Physics physics) {
        ...
        if (hitResult.isWorldHit()) {
            ...
            for (BaseVector2i pos :
                    SpiralIterable.clockwise(Vector2i.zero()).maxRadius(blockPlacementComponent.maximumRange).build()) {
                Vector3i adjacentPositionToHitFace =
                        RotationUtils.rotateVector3i(Direction.inDirection(hitResult.getHitNormal()),
                                new Vector3i(pos.getX(), pos.getY(), 1))
                        .add(hitResult.getBlockPosition());
                Vector3i adjacentPosition =
                        RotationUtils.rotateVector3i(Direction.inDirection(hitResult.getHitNormal()),
                                new Vector3i(pos.getX(), pos.getY(), 0))
                        .add(hitResult.getBlockPosition());
            ...
            LocationComponent locationComponent =
                    GazeAuthoritySystem.getGazeEntityForCharacter(characterEntity).getComponent(LocationComponent.class);
            ...
            if (direction.y < 0f) {
                ...
                Vector3i blockUnderneath = new Vector3i(characterLocationComponent.getWorldPosition(),
                        RoundingMode.HALF_UP).subY(1);
                Vector3i previousBlockUnderneath =
                        new Vector3i(blockUnderneath).sub(horizontalLookDirection.getVector3i());
    ...
    @ReceiveEvent
    public void addIncreaseMaxPower(MechanicalPowerToolPartAddedEvent event, EntityRef item,
                                    MechanicalPowerToolIncreaseMaxPowerComponent details) {
        EntityRef toolEntity = event.getToolEntity();
        PotentialEnergyDeviceComponent consumerComponent =
                toolEntity.getComponent(PotentialEnergyDeviceComponent.class);
    ...
    @ReceiveEvent
    public void addDamageAdjacent(MechanicalPowerToolPartAddedEvent event, EntityRef item,
                                  MechanicalPowerToolDamageAdjacentComponent details) {
    ...
    public void damageAdjacent(DoDamageEvent event, EntityRef entityRef, BlockComponent blockComponent) {
        ...
        if (!toolEntity.equals(event.getInstigator())) {
            ToolDamageAdjacentComponent damageAdjacentComponent =
                    toolEntity.getComponent(ToolDamageAdjacentComponent.class);
            if (damageAdjacentComponent != null) {
                ...
                if (!beforePowerToolUsedEvent.isConsumed() && beforePowerToolUsedEvent.isToolPowered() && event.getInstigator().hasComponent(CharacterComponent.class)) {
                    ...
                    if (result.isWorldHit()) {
                        // loop through each of the directions to damage, reorienting each with what side the block
                        // was hit on.
                        for (Vector3i relativePosition : damageAdjacentComponent.directions) {

                            Vector3i originPosition = blockComponent.getPosition();
                            Vector3i adjacentPosition =
                                    RotationUtils.rotateVector3i(Direction.inDirection(result.getHitNormal()),
                                            relativePosition).add(originPosition);

                            EntityRef adjacentEntityRef = blockEntityRegistry.getBlockEntityAt(adjacentPosition);
                            DoDamageEvent adjacentEvent = new DoDamageEvent(event.getAmount(), event.getDamageType(),
    ...
    @ReceiveEvent
    public void canUsePowerTool(BeforePowerToolUsedEvent event, EntityRef toolEntity,
    ...
    @ReceiveEvent
    public void usePowerTool(PowerToolUsedEvent event, EntityRef toolEntity,
                             PotentialEnergyDeviceComponent consumerComponent) {
        float scaledAmount = (float) Math.pow(1.15, event.getAmount());
    ...
    @ReceiveEvent
    public void onUseBlockPlacementTool(ActivateEvent event, EntityRef tool,
                                        ToolBlockPlacementComponent blockPlacementComponent) {
        EntityRef instigator = event.getInstigator();

        List<Vector3i> positions = getPotentialBlockPlacementPositions(blockPlacementComponent, instigator,
                worldProvider, physics);
        if (positions.size() > 0) {
            ...
            if (!beforePowerToolUsedEvent.isConsumed() && beforePowerToolUsedEvent.isToolPowered() && instigator.hasComponent(CharacterComponent.class)) {
                EntityRef ammoChest = getAmmoChestForBlockPlacementTool(instigator, blockPlacementComponent,
                        positions.size());
    ...
    private EntityRef getAmmoChestForBlockPlacementTool(EntityRef instigator,
                                                        ToolBlockPlacementComponent blockPlacementComponent,
                                                        int itemsNeeded) {
        EntityRef targetItem = EntityRef.NULL;
        for (EntityRef item : ExtendedInventoryManager.iterateItems(inventoryManager, instigator)) {
            BlockPlacerAmmoChestComponent blockPlacerAmmoChestComponent =
                    item.getComponent(BlockPlacerAmmoChestComponent.class);
    ...
    @ReceiveEvent
    public void retainBlockPlacerAmmoChestToItem(OnBlockToItem event, EntityRef entityRef,
                                                 BlockPlacerAmmoChestComponent blockPlacerAmmoChestComponent) {
